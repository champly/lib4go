...
func TestPopAll(t *testing.T) {
	...
	t.Log(m.PopAll())
	if m.Count() != 0 {
		t.Error("after pop all, cmap count must be 0, but :", m.Count())
	}
}

func TestSetIfAbsCb(t *testing.T) {
	m := New(ShareCount)
	k := "elephant"
	tt := time.Now().Format("20060102150405")
	// first
	ok, v, err := m.SetIfAbsentCb(k, func(key string, input ...interface{}) (interface{}, error) {
		return fmt.Sprintf("%s-%s", key, input[0]), nil
	}, tt)
	if ok {
		t.Error("key not exists, but return true")
	}
	if err != nil {
		t.Error("setifabsentcb fail:", err)
	}
	if v.(string) != fmt.Sprintf("%s-%s", k, tt) {
		t.Errorf("expect is:%s, actual is:%s", fmt.Sprintf("%s-%s", k, tt), v.(string))
	}
	...
	if !ok {
		t.Error("key exists, but return false")
	}
	if err != nil {
		t.Error("setifabsentcb fail:", err)
	}
	...
	// error
	ok, _, err = m.SetIfAbsentCb("errors", func(key string, input ...interface{}) (interface{}, error) {
